...
\red{Network Architecture - LSTM layers use tanh activation for cell and hidden state updates and sigmoid activation for gates. Fully connected layers follow standard dense layer configurations. Weights are initialized using Glorot (Xavier) initialization {\cite{DL_FC.2010}}, and biases are zero-initialized.
Optimization - All networks are trained using the Adam optimizer with an initial learning rate of 0.001 and gradient decay factor 0.9.
...
Training Configuration - Networks are trained for a maximum of 30 epochs with a mini-batch size of 50. Validation is performed every 50 iterations with early stopping (patience = 5 epochs).
The model with the lowest validation loss is selected as the final network.
Sequence Handling - LSTM sequences are padded to the length of the longest sequence in each mini-batch.
To minimize padding overhead and potential noise, training sequences are sorted by length prior to mini-batch formation, as this significantly reduces the amount of zero-padding required per batch.}
...
\red{The introduced $O_{\max}=288$-dimensional vector} is used to represent the class in binary form in a vector.
% MD: Before this sentence ended with " and thus generate a 1-to-$O_{\max}$ code". I took this out. @Christian Plase check.
