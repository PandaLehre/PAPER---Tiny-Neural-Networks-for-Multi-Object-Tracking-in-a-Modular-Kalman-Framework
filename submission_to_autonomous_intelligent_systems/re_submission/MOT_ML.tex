...
\end{equation}
where $k$ is the length of the predicted state vector (here $k=5$), $Z_i$ are the entries of the ground truth state vector (KITTI cars and vans tracks) and ${X}_i$ the respective entries of the predicted state vector from our network.
...
The best performing architecture was experimalty determined to be a \ac{BILSTM} layer as shown in Fig. {\ref{pic::SantArch}}.
\begin{figure}[t!]
...
\end{equation}
Then, all scalars obtained per timestamp are summarized and divided by the number of samples $N$ of a minibatch for the cost function:
\begin{equation}
