...
\begin{figure}[!ht]
	...
	\caption{
This schematic representation shows the integration of two \acp{NN} (highlighted in dark blue) within a \ac{TbD} framework.
...
In the commonly used \ac{TbD} paradigm, a tracker fuses detected \acfp{SO} to create consistent object tracks over time.
...
Another key challenge for \ac{TbD} trackers is data association.
...
The temporal information in tracks can, for example, be leveraged through the attention mechanisms as used in {\cite{DL_ATT_CNN_soda.2020, DL_CNN_ATT_mot.2017}} or \acp{RNN} as developed in {\cite{DL_RNN_mot.2016, DL_RNN_data_association.2019}}.
...
Specifically, the work by Mertz et al. {\cite{DL_RNN_data_association.2019}} focuses on data association within a \ac{TbD} framework.
...
\paragraph{Attention-based Tracker}
In this paper, we analyze tracks using \ac{LSTM}-based models.
An alternative architecture would be the attention mechanism {\cite{ML_Attention.2017}}, utilized in various studies {\cite{DL_CNN_ATT_mot.2017,DL_ATT_CNN_soda.2020,DL_CNN_ATT_mot.2017,DL_ATT_Trackformer.2022,Carion2020EndtoEndOD}}.
...
To simulate measurement uncertainty while preserving a controlled training setup, we perturb the ground-truth sensor object states with additive noise. Specifically, we apply zero-mean, independent Gaussian noise to each state component of the incoming sensor objects.
...
The development of \ac{SANT} demonstrated that data-driven association logic can be effectively learned by a deep learning model.
...
To summarize the operational flow of the proposed \ac{TbD} framework and to support reproducibility, table {\ref{tab::pseudoalgo}} provides a concise pseudo-code representation of the overall tracking cycle integrating \ac{SPENT}, \ac{SANT}, and \ac{MANTa}.

\begin{table}[h]
    \caption{Tracking-by-Detection Cycle with SPENT, SANT, and MANTa}
    \begin{algorithmic}[1]
        \REQUIRE Tracks ${T}_{t,1:n}$, sensor objects ${Z}_{t,1:m}$
        \ENSURE Confirmed tracks ${T^c}_{t,1:n}$
        \STATE Predict all track states ${X}_{t,1:n}$ using SPENT
            \IF{SANT is used}
                \STATE Iteratively associate sensor objects ${Z}_{t,1:m}$ to confirmed tracks ${T^c}_{t,1:n}$ to receive ${A}_{t,1:m}$
            \ELSIF{MANTa is used}
                \STATE Jointly associate sensor objects ${Z}_{t,1:m}$ to confirmed tracks ${T^c}_{t,1:n}$ to receive ${A}_{t,1:m}$
            \ENDIF
        \STATE Update associated tracks ${T^u}_{t,1:n}$
        \STATE Perform track confirmation and deletion
        \RETURN ${T^u}_{t,1:n}$
    \end{algorithmic}
    \label{tab::pseudoalgo}
\end{table}

The pseudo-code explicitly distinguishes between iterative single-measurement association with \ac{SANT} and joint multi-measurement association with \ac{MANTa}, reflecting the framework-level selection strategy already described conceptually in Fig. {\ref{pic::MotFramework}}.

\section{Experimental Evaluation}
The developed networks were modularly integrated into the \ac{TbD} framework, replacing classical algorithms.
...
\begin{table}[b]
    ...
    \label{tab::dataAssociationComparison}
\end{table}
